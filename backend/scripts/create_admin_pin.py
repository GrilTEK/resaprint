...
Usage (from inside the api container, e.g. `docker compose exec api ...`):
    ...
    python scripts/create_admin_pin.py --label "Front desk" --pin 1234 --role reception

If a PIN with the same label already exists, it is updated in place
(new hash, role, reactivated, lockout cleared) rather than duplicated.
"""
...
from app.models.admin_pin import AdminPin, AdminRole  # noqa: E402
from app.services.auth_service import hash_pin  # noqa: E402


async def create_or_update_pin(label: str, pin: str, role: AdminRole) -> None:
    async with async_session_maker() as db:
        ...
        if existing:
            ...
            db.add(AdminPin(label=label, pin_hash=hash_pin(pin), role=role))
            action = "Created"

        await db.commit()

    print(f"{action} {role.value} PIN for '{label}'.")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="admin (full access) or reception (reservations + print jobs only). Default: admin.",
    )
    ...
    asyncio.run(create_or_update_pin(args.label, pin, AdminRole(args.role)))
